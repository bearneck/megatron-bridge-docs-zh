# 软件组件版本

## NeMo 框架 26.02

| 软件组件 | 版本 |
|-------------------|---------|
| PyTorch | 2.10.0a0 |
| Megatron Core | main:0.16.0 |
| Transformer Engine | 2.12 |
| Megatron-Bridge | 0.3.0 |
| Megatron-FSDP | 0.3.0 |
| Export-Deploy | 0.4.0 |
| Evaluator | 0.1.74 |
| NeMo | 2.7.0 |
| NeMo Run | 0.8.0 |
| Nvidia-ModelOpt | 0.41.0 |
| NVRX | 0.5.0 |
| CUDA | 13.0.2 |
| cuDNN | 9.18.0.50 |
| TRT-LLM | 1.1.0 |
| vLLM | 0.14.1 |

```{note}
NVIDIA NeMo™ 框架训练容器构建于 NVIDIA 优化框架 PyTorch 25.06 容器之上：https://docs.nvidia.com/deeplearning/frameworks/pytorch-release-notes/index.html
```

## NeMo 框架 25.11

| 软件组件 | 版本 |
|-------------------|---------|
| PyTorch | 2.9.0a0 |
| Megatron Core | dev:0.15.0 |
| Transformer Engine | 2.9 |
| Megatron-Bridge | 0.2.0 |
| Megatron-FSDP | 0.2.0 |
| Export-Deploy | 0.3.0 |
| Evaluator | 0.2.0 |
| NeMo | 2.6.0 |
| NeMo Run | 0.7.0 |
| TRT-ModelOpt | 0.37.0 |
| NVRX | 0.4.1 |
| CUDA | 13.0.1 |
| cuDNN | 9.13.1.26 |
| TRT-LLM | 1.1.0a0 |

```{note}
NVIDIA NeMo™ 框架训练容器构建于 NVIDIA 优化框架 PyTorch 25.06 容器之上：https://docs.nvidia.com/deeplearning/frameworks/pytorch-release-notes/index.html
```

## NeMo 框架 25.09

| 软件组件 | 版本 |
|-------------------|---------|
| PyTorch | 2.8.0a0+5228986c39.nv25.6 |
| Megatron Core | 0.14.0 |
| Transformer Engine | 2.7.0 |
| Megatron-Bridge | 0.1.0rc3 |
| Megatron-FSDP | 0.1.0 |
| Automodel | 0.1.0 |
| Export-Deploy | 0.2.0 |
| Eval | 0.1.0 |
| NeMo | 2.5.0 |
| NeMo Run | 0.6.0 |
| NeMo Curator | 1.0.0 |
| TRT-ModelOpt | 0.35.1 |
| NVRX | 0.4.1 |
| CUDA | 12.9.1 |
| cuDNN | 9.11.0.98 |
| TRT-LLM (支持但未包含) | 1.0.0 |

```{note}
NVIDIA NeMo™ 框架训练容器构建于 NVIDIA 优化框架 PyTorch 25.06 容器之上：https://docs.nvidia.com/deeplearning/frameworks/pytorch-release-notes/index.html
```